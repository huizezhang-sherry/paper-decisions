...
seldom revisited or replicated with alternative choices. In this work,
we focus on a body of data analysis studies on the effect of particulate
matter on mortality, conducted by researchers worldwide, which naturally
provide alteranative analyes of the same question. We automatically
extract analytic decisions from the published literature into structured
data using Large Language Models (Claude and Gemini). We then proposed a
pipeline to calculate paper similarity based on the semantic similarity
of these extracted decisions and their reasons, and visualize the
results through clustering algorithms. This approach offers an efficient
way to study decision-making practices than traditional interviews. We
also provide insights into the use of LLMs for text extraction tasks and
the communication of analytic choices in data analysis practice.    
\end{abstract}
...
\begin{itemize}
\item
  Some decisions may require linking information across sentences and/or
  paragraphs to identify the correct variable. Example:

  \begin{itemize}
  \tightlist
  \item
    Text: if a paragraph states ``\ldots{} model adjusted for
    overdispersion, to estimate associations between day-to-day
    variations in pollutant concentrations (lag 0--5) and day-to-day
    variations in hospital admission counts for three health outcomes
    separately. A basic model without pollutants was built first,
    \ldots.'', Followed by the next paragraph: ``After assessing the
    effects of single day concentrations (lags 0--5), \ldots{}''
  \item
    Guidance: The temporal decision (``lag 0-5''\,``) refers to
    pollutant concentrations, not the model.
  \end{itemize}
\item
  If a sentence includes multiple decisions that vary by outcome or
  variable, each decision should be treated and recorded separately.
  ...
  \item
    ...
    \tightlist
    \item
      Text: ``resulting in a 4-day pollutant average (lag 0--3) for CVD,
      a 5-day average (lag 0--4) for RD, and a 6-day average (lag 0--5)
      for asthma''
    \item
      Output: three separate decisions of type ``temporal''\,''
      (variable/ decision): 1) CVD/ 4-day average (lag 0--3), 2) RD/
      5-day average (lag 0--4), 3) Asthma/ 6-day average (lag 0--5)
    \end{itemize}
  \item
    Example 2:
    ...
    \tightlist
    \item
      Text: ``smoothing splines of one-day lags of humidity {[}each with
    ...
    \item
      ...
      \tightlist
      \item
        Decision 1: variable: ``humidity'', type: ``temporal'',
        decision: ``one-day lag''
      \item
        Decision 2: variable: ``humidity'', type: ``parameter'',
        decision: ``3 df''
      \end{itemize}
    \end{itemize}
  \item
    ...
    \tightlist
    \item
      Text: ``three- day averaged temperature and dew point temperature
    ...
    \item
      ...
      \tightlist
      \item
        Decision 1: variable: ``temperature'', method: ``NA'', type:
        ``temporal'', decision: ``3-day averaged''
      \item
        Decision 2: variable: ``dew point temperature'', method: ``NA'',
        type: ``temporal'', decision: ``3-day averaged''
      \item
        Decision 3: variable: ``temperature'', method: ``natural cubic
        spline'' , type: ``parameter'', decision: ``3 df''
      \item
        Decision 4: variable: ``dew point temperature'', method:
        ``natural cubic spline'', type: ``parameter'', decision: ``3
        df''
      \end{itemize}
    \end{itemize}
  \end{itemize}
\item
  Temporal terms may indicate parameter choices: words like ``monthly,''
  ``weekly,'' or ``daily'' may appear in sentences, but they do not
  always imply temporal decisions. Sometimes they describe how a
  smoothing function is applied, which is a parameter decision. Example:
  ...
  \item
    Interpretation: This describes a spline function applied to time,
    using 12 knots (one per month)
  \item
    Output: This is a \emph{parameter} decision, not a temporal one.
  \end{itemize}
\item
  Please use cognitive reasoning to determine whether the extracted
  reason is \emph{specific} to the decision made. If the reason is
  general and doesn't justify the particular choice, write ``NA''.
  Example:

  \begin{itemize}
  \tightlist
  \item
    Text: ``To control for weather variables, the 14-day lagged moving
    average (including concurrent day and previous 13 days) of
    temperature, dew point temperature, and barometric pressure (as both
    linear and quadratic terms) were included in the conditional
  ...
  \item
    Output: ``NA'' because the reason is general to why the weather
    variables are included and do not justify why this temporal decision
    of 14-day lag is chosen
  \end{itemize}
\item
  Only include rows where a clear decision is made on a parameter,
  spatial, or temporal aspect of the modeling. For ambiguous or vague
  decisions (e.g., ``not below 2 months''), do not record a row.
\item
  Do NOT include decisions about inclusion of variables, for example,
  reasons as ``to filter out cyclical patterns'' and decision as
  ``include day-of-the-week variable.
\item
  A paper may contain multiple models; label them precisely (e.g.,
  ``generalized additive Poisson regression'', ``lag distributed
  model'').
\item
  Convert numbers written in words to numerals. For example, rewrite
  ``one-day lag'' as ``1-day lag''. Apply this consistently across all
  entries. ````
\end{itemize}
...
From the 57 studies examining the effect of particulate matters
...
(\(\text{NO}_2\)). This yields 273 decisions extracted using Gemini,
averaging approximately 5 decisions per paper.
...
(e.g.~use of five degree of freedom), 57\% of decisions lack a stated
...
\begin{longtable}[]{@{}
  ...
  >{\raggedright\arraybackslash}p{(\linewidth - 4\tabcolsep) * \real{0.6915}}@{}}
...
\begin{longtable}[]{@{}
  ...
  >{\raggedright\arraybackslash}p{(\linewidth - 4\tabcolsep) * \real{0.1412}}
...
single-day lag & humidity & lag 0, lag 1, lag 2, lag 3, lag 4, lag 5,
lag 6, lag 7 \\
single-day lag & temperature & lag 0, lag 1, lag 2, lag 3, lag 4, lag
5 \\

\end{longtable}

\phantomsection\label{cell-fig-cluster-paper-1}
\begin{figure}[H]
...
\phantomsection\label{cell-fig-cluster-paper}
\begin{figure}[H]
...
shows the clustering of the 50 papers based on the decision similarity
...
scores from each model are comparable and \textbf{?@fig-text-mds} shows
the multi-dimensional scaling (MDS) of the paper similarity scores from
each text model: all showing a similar clustering pattern of the three
...
\item
  Spatial decisions are generally not well captured because it often
  conducted uniformly as estimating the city individually to accommodate
  city heterogeneity. Some papers only consider a handful of cities,
  while in larger studies the individual city effects are then pooled
  together using random effect.
\item
...
\begin{itemize}
\item
  some decisions are more varied than others and can be reported by
  different ways. e.g.~the most common way to report the smoothing
  parameter for time is the number of knots/ degree of freedom per year.
  While authors may report this number in different ways, i.e.~``every
  30 days''. A secondary processing with LLM may be useful to align the
  raw text into the same reporting unit.s
\item
  the variation of parameters people use can help to identify parameter
  that needs a separate sensitivity analysis
\end{itemize}
