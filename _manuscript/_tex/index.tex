...
language models tested include 1) standard \texttt{BERT}
(\texttt{google-bert/bert-base-uncased}) \citep{devlin2019}, 2)
\texttt{Roberta} (\texttt{FacebookAI/roberta-base}) \citep{liu}, which
is trained on a much larger dataset (160GB v.s. BERT's 15GB), 3)
\texttt{xlnet} by Google Brain (\texttt{xlnet/xlnet-base-cased})
\citep{yang}, and two domain-trained BERT models: 4) \texttt{sciBert}
(\texttt{allenai/scibert\_scivocab\_uncased}) \citep{beltagy2019},
trained on scientific literature, and 5) \texttt{bioBert}
(\texttt{dmis-lab/biobert-large-cased-v1.1-squad}) \citep{lee2020},
