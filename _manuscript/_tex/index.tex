...
\subsection{Air pollution mortality
modelling}\label{air-pollution-mortality-modelling-1}
...
\caption{\label{tbl-missing-decisions}THis is hte caption}

\centering{

\begin{tabular}{lll}
\toprule
\multicolumn{1}{c}{} & \multicolumn{2}{c}{Missing decision} \\
\cmidrule(l{3pt}r{3pt}){2-3}
Missing reason & FALSE & TRUE\\
\midrule
FALSE & 95 (32.4\%) & 13 (4.4\%)\\
TRUE & 179 (61.1\%) & 6 (2\%)\\
\bottomrule
\end{tabular}
...
The results follows examines 57 papers for modelling the effect of
particulate matters on mortality based on Gemini for parsing the
decision choices. The results from Anthropic Claude is reported in the
sensitivity analysis section in Section~\ref{sec-sensitivity}. From the
57 papers, we extracted 273 decisions (\textasciitilde{} 5 decisions
each paper). Out of which the most are parameter-related decisions and
temporal decision, accounting for 1\% and 43\%, respectively.
Table~\ref{tbl-missing-decisions} summarizes the missingness of the
decisions and reason for each record. While most papers provide the
choice of their decision (e.g.~use of 5 degree of freedom), a 61\% of
the decisions are missing the reason for the choice. This reveals a
prevalent phenomenon that authors tend to not justify the choice being
made, which requires attention. We also observe data quality with the
extraction: for example in \citet{lee2006}, the variable recorded is
``smoothing parameter''. Authors are unclear about the delivery Specify
how much of validation and review has been done.

\begin{longtable}[]{@{}llr@{}}

\caption{\label{tbl-most-common-decisions}Count of xxx in the Gemini
dataset}
...
humidity & temporal & 18 \\
PM & parameter & 7 \\
time & temporal & 5 \\

\end{longtable}

Table~\ref{tbl-most-common-decisions} shows the most common decision for
air pollution modelling is to the parameter choice for time, PM,
temperature, and humidity. Given the count of decisions included by all
the papers, we decide to include the first 6 most common variable-type
decisions in the analysis to calculate the decision similarities.

Table~\ref{tbl-humidity-temperature-decisions} shows number of knots for
the natural spline and degree of freedom for smoothing spline for time,
humidity and temperature variables. {[}TODO{]}

\begin{itemize}
\item
  The choice of knot for natural spline is less varied than the choice
  of degree of freedom for smoothing spline
\item
  The choice for temperature and humidity tends to be the same,
  potentially due to they are both weather related variables.
\item
  The choice for time is more varied. and we observe interesting choices
  of 7.7 in \citet{castillejos2000} and flexible choice of 30 and 100 in
  \citet{moolgavkar2000} and \citet{moolgavkar2003}, respectively.
\item
  most paper choices a fix number for the smoothing parameter while
  \citet{schwartz2000} choose a proportion of the data: ``50\% of the
  data'', ``5\% of the data''
\end{itemize}

Table~\ref{tbl-temporal-decisions} shows \ldots.

{[}look at for one type of decision (time) - what are the choices made
by different papers{]}
...
\caption{\label{tbl-humidity-temperature-decisions}Most common decisions
for humidity and temperature variables in the Gemini dataset}
...
\caption{\label{tbl-temporal-decisions}Temporal lag decisions for PM,
temperature, and humidity variables in the Gemini dataset}
...
Figure~\ref{fig-cluster-paper} shows the clustering of the 57 papers
based on the decision similarity scores. The dendrogram is generated
using hierarchical clustering, and the labels are colored according to
the most common smoothing method used in each paper. The clustering
reveals three distinct groups of papers, which reflect the modelling
strategies differ in the European and U.S. studies {[}more on the
APHENA{]}. The first group (left) primarily uses LOESS smoothing, while
the second group (middle) employs natural splines, and the third group
(right) uses smoothing splines.

\begin{itemize}
\tightlist
\item
  choose the ones in the baseline model
\item
  don't include other options considered, only the final value used in
  the model
\item
  no sensitivity analysis
\item
  only consider PM2.5 and PM10 not other pollutants
\end{itemize}
...
all 62 papers in Figure~\ref{fig-claude-gemini}. Each point represents a
...
\phantomsection\label{cell-fig-claude-gemini}
\begin{figure}[H]
...
same number of decisions. Most points fall below this line, suggesting
