...
\hypersetup{
  pdftitle={A LLM-based pipeline for understanding decision choices in data analysis from published literature},
...
\title{A LLM-based pipeline for understanding decision choices in data
...
%% article.
\begin{abstract}
Decision choices, such as selecting the degree of freedom for a
smoothing spline and the rationale behind them, are essential for
interpreting results and understanding how uncertainty propagates
through an analysis. Understanding how such choices are made and
identifying common patterns in decisions can help junior researchers
learn the field, while offering domain researchers a survey-like
overview of current analysis practices. However, these decisions are
rarely studied, as it is impractical to interview authors about all the
alternatives considered during the work or to rerun the analysis under
different options. In this work, We propose a workflow to automatically
extract analytic decisions and their stated reasons from published
literature using Large Language Models (Claude and Gemini). The pipeline
also contributes to the calculation of paper similarity based on the
semantic similarity of these extracted decisions and their reasons, and
the visualization of papers using clustering algorithms. We apply this
workflow to a set of studies on the effect of particulate matter on
mortality and hospital admission, conducted by researchers worldwide,
which naturally provide alteranative analyses of the same question. This
approach enables large-scale, automated studies of decision-making
practices in data analysis, providing an alternative to existing manual
interview-based studies and small-scale `many analysts experiments'.    
\end{abstract}
...
\item
  A shiny GUI tool for validation LLM outputs in this context, and
\item
...
\pandocbounded{\includegraphics[keepaspectratio]{index_files/figure-pdf/fig-hc-1.pdf}}

}

\caption{\label{fig-hc}Maybe we don't need teh hierarchical clustering
plot because it has the same information as the next MDS one???}
...
distance matrix is then used for hierarchical clustering in
Figure~\ref{fig-hc} and multi-dimensional scaling (MDS) in
...
for each model for cross-comparison among papers. Apart from modelling
choices, other decisions in data pre-processing are also worth
comparing. This would include how variables are defined and computed
from the raw data.
